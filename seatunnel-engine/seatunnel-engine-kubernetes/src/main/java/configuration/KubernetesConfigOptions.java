...
public class KubernetesConfigOptions {
    ...
    //                                    + "The exposed rest service could be used to access the
    // Flink’s Web UI and REST endpoint.");
    ...
    //                                    Description.builder()
    //                                            .text(
    //                                                    "The user-specified %s that is used for
    // filtering node IPs when constructing a %s connection string. This option is only considered
    // when '%s' is set to '%s'.",
    //                                                    link(
    //
    ...
    public static final ConfigOption<String> KUBE_CONFIG_FILE =
            key("kubernetes.config.file")
                    .stringType()
                    .defaultValue("/Users/zhuoyuchen/.kube/config")
                    .withDescription(
